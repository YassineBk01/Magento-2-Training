...
class AfterModelLoadedObserver implements ObserverInterface
{
    protected $logger;
    ...
    public function execute(Observer $observer)
    {
        ...
        try {
            ...
            $userAction->setActionType('Load Action');
            $userAction->setAffectedModule('$moduleName');

            $userAction->save();

            $this->logger->info('Failed Load activity logged for username: ' . $object->getUsername());
        } catch (\Exception $e) {
            $this->logger->error('Error logging Load activity: ' . $e->getMessage());
        }
    }
}
